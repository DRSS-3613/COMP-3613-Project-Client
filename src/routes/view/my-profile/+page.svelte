...
<div class="w-full bg-white">
	<header class="w-full flex flex-wrap items-center p-4 md:py-8">
		...
		<div class="w-8/12 md:w-7/12 ml-4">
			<div class="md:flex md:flex-wrap md:items-center mb-4">
				...
				<button
					class="bg-blue-500 px-2 py-1 
                        ...
                        sm:inline-block">Edit Profile</button
				>
			</div>
			...
			<ul class="hidden md:flex space-x-8 mb-4">
				<li>
					<span class="font-semibold">{$currentUser.images.length}</span>
					posts
				</li>

				<li>
					<span class="font-semibold">{$currentUser.ratings.length}</span>
					Rates
				</li>
				<li>
					<span class="font-semibold">{$currentUser.averageRating}</span>
					Rating
				</li>
			</ul>
		</div>
	...
	<div class="px-px md:px-3">
		...
		>
			<li>
				<span class="font-semibold text-gray-800 block">{$currentUser.images.length}</span>
				posts
			</li>

			<li>
				<span class="font-semibold text-gray-800 block">{$currentUser.ratings.length}</span>
				Rates
			</li>
			<li>
				<span class="font-semibold text-gray-800 block">{$currentUser.averageRating}</span>
				Rating
			</li>
		</ul>
		<!-- flexbox grid -->
		<div class="flex flex-wrap -mx-px md:-mx-3">
			{#each $currentUser.images as post, index}
				<!-- column -->
			...
			{#if $currentUser.images.length < 4}
				<!-- content here -->
				{#each Array(4 - $currentUser.images.length) as _}
					<!-- column -->
					<div class="w-1/3 p-px md:px-3">
						<!-- post 1-->
						<article class="post bg-gray-100 text-white relative pb-full md:mb-6">
